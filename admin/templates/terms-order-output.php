<div class="wrap">
                <h2><?php _e( "Terms Order", 'terms-order' ) ?></h2>
                
                <div id="ajax-response"></div>
                
                <noscript>
                    <div class="error message">
                        <p><?php _e( "Jquery library required to this plugin work.", 'terms-order' ) ?></p>
                    </div>
                </noscript>
                ...
                <form action="<?php echo esc_attr($current_section_parent_file); ?>" method="get" id="tt_terms_order_form">
                    <input type="hidden" name="page" value="ttto-<?php echo esc_attr($post_type) ?>" />
                    <?php
                    ...
                    if (count($post_type_taxonomies) > 1)
                        {
                            ...
                            <h2 class="subtitle"><?php echo ucfirst($post_type_data->labels->name) ?> <?php _e( "Taxonomies", 'terms-order' ) ?></h2>
                            <table cellspacing="0" class="wp-list-taxonomy">
                                <thead>
                                <tr>
                                    <th style="" class="column-cb check-column" id="cb" scope="col">&nbsp;</th><th style="" class="" id="author" scope="col"><?php _e( "Taxonomy Title", 'terms-order' ) ?></th><th style="" class="manage-column" id="categories" scope="col"><?php _e( "Total Posts", 'terms-order' ) ?></th>    </tr>
                                </thead>
                                    ...
                                    foreach ($post_type_taxonomies as $post_type_taxonomy){
                                            ...
                                            ?>
                                                <tr valign="top" class="<?php if ($alternate === TRUE) {echo 'alternate ';} ?>" id="taxonomy-<?php echo esc_attr($taxonomy)  ?>">
                                                        <th class="check-column" scope="row"><input type="radio" onclick="tt_terms_order_change_taxonomy(this)" value="<?php echo esc_attr($post_type_taxonomy) ?>" <?php if ($post_type_taxonomy == $taxonomy) {echo 'checked="checked"';} ?> name="taxonomy">&nbsp;</th>
                ...
                <div id="order-terms">
                    ...
                    <div id="post-body">                    
                            ...
                            <ul class="sortable" id="tt_term_sortable">
                                <?php $instance->ttto_terms_list($taxonomy); ?>
                            </ul>
                    ...
                    <div class="actions">
                        <p class="submit">
                            <a href="javascript:;" class="save-order button-primary"><?php _e( "Update", 'terms-order' ) ?></a>
                        </p>
                    </div>
                    ...
                    jQuery(document).ready(function() {
                        ...
                        jQuery(".save-order").bind( "click", function() {
                                ...
                                jQuery.post( ajaxurl, { action:'update-taxonomy-order', order: serialize_data, nonce : '<?php echo wp_create_nonce( 'update-taxonomy-order' ); ?>' }, function() {
                                    jQuery("#ajax-response").html('<div class="message updated fade"><p><?php _e( "Items Order Updated", 'terms-order' ) ?></p></div>');
